...
class AdminListing
{
    ...
    public function processRequestAndGetAAA(Request $request, array $columns = ['*'], $searchIn = null, callable $modifyQuery = null, $locale = null, $options)
    {
        // process all the basic stuff
        $this->attachOrdering($request->input('orderBy', $this->model->getKeyName()), $request->input('orderDirection', 'asc'))
            ->attachSearch($request->input('search', null), $searchIn);
        ...
        // If the options array is filled we use the information coming from it, else we use the default values
        !isset($options['orderBy']) ? $orderBy = $this->model->getKeyName() : $orderBy = $options['orderBy'];
        !isset($options['orderDirection']) ? $orderDirection = 'asc' : $orderDirection = $options['orderDirection'];
        // process all the basic stuff
        $this->attachOrdering($request->input('orderBy', $orderBy), $request->input('orderDirection', $orderDirection))
            ->attachSearch($request->input('search', null), $searchIn);
        // if bulk filter is enabled we want to get only primary keys
        if ($request->input('bulk')) {
            return $this->get(['id']);
